...
| Wi-Fi | 按需 2.4 GHz STA 扫描及由应用管理的 STA 连接 | `main/demo_wifi.c`、`main/vocabulary_client.c`、`main/asr_audio.c` | 雅思背单词与 Voice ASR 的凭据通过本地 USB 配置并存入 NVS；天线/射频表现仍未验证 |
...
应用也可以使用 ESP-IDF 提供的定时器、FreeRTOS 任务和内部 Flash/NVS；番茄钟分支提供了 NVS 示例。Wi-Fi 和 Bluetooth LE 仍是 ESP-IDF 应用服务而非 BSP API：菜单页面负责其协议栈生命周期，联网应用则在工作任务中执行耗时网络操作。`demo/claude-buddy-port` 仍是更完整的 BLE 应用架构参考，不能替代对当前板卡天线、射频表现、功耗和共存行为的实测。当前产品与固件基线使用 8 MB Flash，包含 3 MB factory-app 分区，并固定保留受保护的 `cardid` 存储区。
