...
- 将主菜单中的 Display 演示替换为雅思背单词 MVP。设备使用本地配置的手机热点联网，通过 HTTPS 获取有数量上限的学习会话，支持释义揭示、认识、不熟悉与前后导航，保存读音偏好，并在工作任务中幂等提交学习进度。新增可本地部署的 FastAPI/SQLite 服务、令牌保护的进度页面、隔离的 Docker 部署文件，以及设备和服务端状态转换的主机测试。单词读音播放、完整且有授权的雅思词表及持久化离线事件存储留待后续实现。
